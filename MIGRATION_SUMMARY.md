...
The deployment mechanism has been successfully migrated from the legacy PowerShell script (`scripts/deploy.ps1`) to **Azure Developer CLI (azd)** with **Bicep Infrastructure-as-Code**.
...
- **`azd.yaml`** — Project manifest defining services (frontend/backend), provisioning step, and outputs
- **`infra/main.bicep`** — Main Bicep template with all Azure resources and deterministic naming
- **`infra/modules/webapp.bicep`** — Reusable module for App Service creation with logging
- **`infra/modules/sql.bicep`** — SQL Server/DB module with Managed Identity and Azure AD auth
- **`infra/parameters.json`** — Default parameters for local development

### Supporting Scripts & Docs
- **`scripts/azd-post-provision.ps1`** — Post-provision hook that generates `env-config.txt` from azd outputs
- **`README.md`** — Updated with azd workflow documentation
- **`.github/copilot-instructions.md`** — Updated with new IaC approach and Managed Identity details
...
- Existing `scripts/simulate-issues.ps1` and `scripts/generate-load.ps1` scripts continue to work unchanged
- Legacy `scripts/deploy.ps1` still available (deprecated, but supported)
...
4. **Generate Load**: Run `.\scripts\simulate-issues.ps1 -Issue all-issues`
5. **Test Scripts**: Run `.\scripts\generate-load.ps1 -Scenario all` and verify App Insights data
...
- **Deployment Command**: `.\scripts\deploy.ps1` → `azd provision`

---

## ⚙️ Naming Constraints Enforced

- **Storage Account**: Lowercase, 3–24 chars, alphanumeric (special handling: `st{env}{token}` with normalized casing)
...
- ✅ Secure (no embedded passwords, Managed Identity)
- ✅ Reproducible (deterministic naming)
- ✅ Scalable (parameterized, modular Bicep)
- ✅ Backwards compatible (existing scripts still work)

Proceed to testing in a non-production Azure subscription. 🚀
