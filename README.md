...
| Managed Identity | `mi-copilot-demo-demo-abc1` | User-assigned identity for Web Apps → SQL authentication |
| App Service Plan | `asp-copilot-demo-demo-abc1` | Hosts both web apps (B1 tier, shared) |
| Frontend Web App | `app-frontend-demo-abc1` | UI served by Node.js 18 (from `services/frontend`) |
| Backend API Web App | `app-backend-demo-abc1` | REST API served by Node.js 18 (from `services/backend`) |
| SQL Server | `sql-demo-abc1` | Azure SQL logical server (Entra ID / Azure AD authentication) |
| SQL Database | `appdb` | Application database (Basic tier, 5 DTU) |
| Application Insights | `ai-copilot-demo-demo-abc1` | Full-stack tracing and monitoring |
| Storage Account | `stdemoabc1` | Logs and diagnostics (Standard LRS) |
...
│   └── modules/
│       ├── webapp.bicep              # Reusable App Service module
│       └── sql.bicep                 # SQL Server + Database module
├── services/                         # Application code (auto-deployed by azd)
│   ├── frontend/
│   │   ├── package.json              # Node.js 18 dependencies
│   │   └── app.js                    # Express-like HTTP server with UI
│   └── backend/
│       ├── package.json              # Node.js 18 dependencies
│       └── app.js                    # REST API with health checks, data endpoints
├── scripts/                          # Troubleshooting & demo automation (PowerShell)
│   ├── deploy.ps1                    # Legacy: Manual resource creation (deprecated)
│   ├── azd-post-provision.ps1        # Runs after azd provision; generates env-config.txt
│   ├── simulate-issues.ps1           # Create observable problems for demos
│   └── generate-load.ps1             # Populate Application Insights with traffic
├── env-config.txt                    # Auto-generated by post-provision hook; sourced by scripts
└── DEMO-PROMPTS.md                   # Validated Copilot prompts for the demo
```
